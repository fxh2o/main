(() => {
  ...
  function injectHeaderNavStyles() {
    ...
    style.textContent = `
      ...
      .headerCenterNav .headerNavButton { position: relative; width: auto !important; min-width: 72px; height: 38px !important; padding: 0 10px !important; display: inline-flex !important; align-items: center !important; justify-content: center !important; border: 1px solid transparent !important; border-radius: 999px !important; background: transparent !important; color: var(--muted) !important; font-size: 13px !important; font-weight: 700 !important; line-height: 1.2 !important; text-decoration: none !important; cursor: pointer !important; transition: color .16s, background .16s, border-color .16s !important; }
      .headerCenterNav .headerNavButton:hover, .headerCenterNav .headerNavButton:focus-visible { border-color: rgba(200,255,0,.25) !important; background: rgba(200,255,0,.08) !important; color: var(--text) !important; outline: none !important; }
      .headerCenterNav .headerNavButton.active { border-color: rgba(200,255,0,.35) !important; background: rgba(200,255,0,.13) !important; color: var(--accent) !important; }
  ...
  function setupHeaderNavigation() {
    ...
    headerHomeButton.classList.toggle('active', location.hash !== '#library');
  ...
  async function showLibrary(push = true) {
    ...
    headerHomeButton.classList.remove('active');
    app.setView?.('library');

    if (typeof app.loadAllAnime === 'function') {
      list.innerHTML = '<div class="librarySkeleton" role="status" aria-label="Loading library"><span></span><span></span><span></span><span></span><span></span><span></span></div>';
